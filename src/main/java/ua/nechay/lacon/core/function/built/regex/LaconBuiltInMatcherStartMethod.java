 ...
 * @since 19.05.2023
 */
public class LaconBuiltInMatcherStartMethod extends FunctionLaconValue {
    private static final String GROUP_ARGUMENT = "group";

    private static final LaconBuiltInMatcherStartMethod INSTANCE = new LaconBuiltInMatcherStartMethod(List.of(
        ...
        try {
            return new IntLaconValue(matcher.start((int) group));
        } catch (Exception e) {
            return new IntLaconValue(-1);
        }
    }, LaconBuiltInType.INT);

    public LaconBuiltInMatcherStartMethod(@Nonnull List<LaconFunctionArgument> args,
    ...
    public static LaconBuiltInMatcherStartMethod getInstance() {
        return INSTANCE;
    }
}
